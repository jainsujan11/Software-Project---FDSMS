...
client = MongoClient("mongodb+srv://wwwphy2003:6tmVgV8bwV1uvBJo@cluster0.1z5x1kd.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
def placeOrder():
    global discount
    session['currentOrderCreating']['discountValue'] = discount
    currentOrder = session['currentOrderCreating']
    db = client.Orders
...
def moreDetailsOrder(orderId):
    ...
    if currentOrder['offerId']:
        db = client.Management
        offerUsed = db['Offers'].find_one({'offerId': (currentOrder['offerId'])})
        if offerUsed:
...
def seeDeliveryRequest():
    if session['sessionUser']['userType'] != 'deliveryAgent':
        return redirect(url_for('logout'))
    deliveryID = session['userId']
    deliveryID = deliveryID[19:-4]
    deliveryID = ObjectId(deliveryID)
    area = client.DeliveryAgent['User_Info'].find_one({'_id': deliveryID})['area']
    db = client.Area
...
def acceptDeliveryRequest():
    ...
    client.Orders['OrdersPlaced'].update_one({'_id': ObjectId(order_id)}, {'$push': {
        'orderUpdates': {
...
def moreDetailsDeliveryRequest(status):
    ...
    if status != "NoOrder":
        ...
        if current_order:
            ...
            final = current_order['orderValue']-discount+deliveryCharge

    return render_template('moreDetailsDeliveryAgent.html', customerName=customerName, restaurantName=restaurantName, address = address, orderList=orderList, cost= cost, discount=discount, deliveryCharge=deliveryCharge, final= final, showButton = showButton, printTable = printTable,temp=temp)
